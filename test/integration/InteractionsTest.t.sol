...
contract InteractionsTest is ZkSyncChainChecker, StdCheats, Test {
    ...
    function setUp() external {
        // 1. สร้าง Instance ของ Script ที่ใช้ Deploy
        DeployFundMe deployer = new DeployFundMe();

        // 2. เรียกฟังก์ชัน deployFundMe() เพื่อให้ได้ Contract instance มาใช้งาน
        (fundMe, helperConfig) = deployer.deployFundMe();

        // 3. ให้เงิน (ETH) กับ USER จำลอง เพื่อใช้สำหรับเทส
        vm.deal(USER, STARTING_USER_BALANCE);
    }

    function testUserCanFundAndOwnerWithdraw() public skipZkSync {
        // 1. ลองใช้ Script ในการ Fund (Integration Style)
        FundFundMe fundFundMe = new FundFundMe();
        // เราส่ง address(fundMe) เข้าไปให้ script ทำงาน
        fundFundMe.fundFundMe(address(fundMe));

        // 2. ลองใช้ Script ในการ Withdraw
